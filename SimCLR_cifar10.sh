#!/bin/bash
#SBATCH --job-name=SimCLR_C10
#SBATCH --account=def-pfieguth
#SBATCH --time=10:00:00
#SBATCH --mem-per-cpu=10G
#SBATCH --gpus=nvidia_h100_80gb_hbm3_2g.20gb:1
#SBATCH --output=slurm_output/SimCLR_C10.out
#SBATCH --error=slurm_output/SimCLR_C10.err
...
for SEED in "${SEEDS[@]}"; do
  for NOISE in "${NOISE_RATES[@]}"; do
    EXP_NAME="baseline_c10_noise-${NOISE}_s${SEED}"
...
for SEED in "${SEEDS[@]}"; do
  for E in "${EPOCHS[@]}"; do
    EXP_NAME="simclr_c10_e${E}_s${SEED}"
    ...
    python ssl_cifar_experiment.py \
      ...
      --pretrain-epochs "${E}" \
      --exp-name "${EXP_NAME}" \
      --save-pretrained-encoder "simclr_c10_e${E}_s${SEED}"
...
for SEED in "${SEEDS[@]}"; do
  for E in "${EPOCHS[@]}"; do
    for NOISE in "${NOISE_RATES[@]}"; do
        ...
        EXP_NAME="simclr_c10_e${E}_s${SEED}_noise-${NOISE}"
        ...
        --noise-rate "${NOISE}"\
        --exp-name "${EXP_NAME}" \
        --pretrained-encoder-path "pretrained_encoders/simclr_c10_e${E}_s${SEED}"
